...
public class AuthenticationInterceptor implements ServerInterceptor {
  ...
  private static Optional<AccessRight> correspondingMethod(
    ...
    return Optional.ofNullable(method.getAnnotation(Authorization.class))
  ...
  private static Method grpcMethodImplementation(
      final BindableService service, final String method) {
    return Arrays.stream(service.getClass().getMethods())
        .filter(m -> matchingGrpcMethod(method, m))
        .reduce(
            (a, b) -> {
              throw new IllegalStateException(
                  "Service has multiple matching gRPC methods for " + method);
            })
        .orElseThrow(() -> new IllegalStateException("No matching gRPC method for " + method));
  }

  /**
   * This method performs magic. The gRPC framework does not expose the implementation method used
   * for the request. Hence, we need to lookup what methods corresponds to the gRPC endpoints.
   *
   * <p>This is only performed on startup-time, so it does not affect runtime performance
   *
